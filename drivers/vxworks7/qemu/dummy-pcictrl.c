...
It Reads configuration header for the setting up the resources (vxbResourceAlloc):
BAR addresses
...
dummy_pci: dummy_pci@0 {
    ...
    // bus-range contains two cells. The first one specifies the bus number of current PCI node, which is 0.
