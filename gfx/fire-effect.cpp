...
struct Fire
{
	...
	Fire(int w, int h, double r) : w(w), h(h), r(r)
	{
		...
		for (x = 0; x < (int)SDL_arraysize(pal); x++)
		{
	...
	void update_and_render(SDL_Renderer *renderer, float)
	{
		...
		for (y = 0; y < h; y++)
		{
			for (x = 0; x < w; x++)
			{
				auto c = pal[fire[y][x] % SDL_arraysize(pal)];
...
int main(int argc, char *argv[])
{
	...
	if (!SDL_CreateWindowAndRenderer("Fire Effect", opt.width, opt.height, SDL_WINDOW_RESIZABLE, &window, &renderer))
		fatal("sdl: %s", SDL_GetError());

	Fire fire(opt.width, opt.height, opt.ratio);

	SDL_SetRenderLogicalPresentation(renderer, opt.width, opt.height, SDL_LOGICAL_PRESENTATION_STRETCH);
