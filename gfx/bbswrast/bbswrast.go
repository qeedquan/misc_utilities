...
func (c *Camera) Rotate(axis f64.Vec4, angle float64) {
	c.Transform = c.Transform.Rotate(f64.Quat{}.FromAxisAngle(axis.XYZ(), angle))
}
...
func (c *Camera) GetViewProjection() f64.Mat4 {
	...
	rotation := c.Transform.Rotation.Mat4()
...
func RotateVector(rotation f64.Quat, vec f64.Vec4) f64.Vec4 {
	qi := rotation.Conj()
	qr := rotation
	qp := qr.Mulv(vec).Mul(qi)
	return f64.Vec4{qp.X, qp.Y, qp.Z, 1}
}

func CardinalDirections(rotation f64.Quat) (forward, backward, up, down, left, right f64.Vec4) {
	forward = f64.Vec4{0, 0, 1, 1}
	backward = f64.Vec4{0, 0, -1, 1}
	up = f64.Vec4{0, 1, 0, 1}
	down = f64.Vec4{0, -1, 0, 1}
	left = f64.Vec4{-1, 0, 0, 1}
	right = f64.Vec4{1, 0, 0, 1}
