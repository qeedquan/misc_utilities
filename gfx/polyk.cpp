...
void init_sdl()
{
	...
	if (!SDL_CreateWindowAndRenderer("Polygon Operations", 800, 800, SDL_WINDOW_RESIZABLE, &window, &renderer))
...
void draw_triangle(Point a, Point b, Point c)
{
	...
	SDL_RenderLine(renderer, a.x, a.y, c.x, c.y);
...
void draw_polyk(Polyk &pl, bool aabb, bool insides, bool triangulate, SDL_Color bordercol)
{
	for (size_t i = 0; i < pl.size(); i++)
	{
		...
		SDL_RenderLine(renderer, pl.points[i].x, pl.points[i].y, pl.points[j].x, pl.points[j].y);
	}
...
void draw_closest_edge(Polyk &pl, Point p)
{
	...
	SDL_RenderLine(renderer, c.closest.x, c.closest.y, p.x, p.y);
}
...
void draw_rand_raycasts(Polyk &pl)
{
	...
	for (size_t i = 0; i < rn; i++)
	{
		...
		SDL_RenderLine(renderer, mx, my, mx + dx * isc.dist, my + dy * isc.dist);
	}
}
