...
func (l *L) Draw(s string) {
	w, h, _ := renderer.OutputSize()
	l.P = ga.Vec2d{float64(w / 2), float64(h)}
	l.P = vec2.Add(l.P, l.O)
	l.Pl = l.Pl[:0]
	l.Plr = false
	for i, op := range s {
		if i > 0 && i%100000 == 0 {
		...
		case '|': // Reverse direction (ie: turn by 180 degrees)
			l.A += -ga.Sign(l.A) * ga.Deg2Rad(180.0)
...
func (l *L) move(dx, dy float64) (p0, p1 ga.Vec2d) {
	p := l.P
	m := mat2.Rotation(l.A)
	l.P = vec2.Add(l.P, mat2.Apply(&m, ga.Vec2d{dx, dy}))
	if l.Plr {
