...
func reset() {
	location = ga.Vec2d{670, 500}
	orientation = 0
	elevation = 120
	distanceFOV = 800
}

func moveLocation(step ga.Vec2d) ga.Vec2d {
	M := mat2.Rotation(orientation)
	dir := mat2.Apply(&M, step)
	return vec2.Add(location, dir)
}
	...
	for {
		...
		case sdl.KeyDownEvent:
			...
			case sdl.K_ESCAPE:
				os.Exit(0)
			case sdl.K_DOWN:
				location = moveLocation(ga.Vec2d{0, step})
			case sdl.K_UP:
				location = moveLocation(ga.Vec2d{0, -step})
			case sdl.K_LEFT:
				location = moveLocation(ga.Vec2d{-step, 0})
			case sdl.K_RIGHT:
				location = moveLocation(ga.Vec2d{step, 0})
			case sdl.K_q:
				orientation -= 0.1
...
func (c *Terrain) hline(m *image.RGBA, p1, p2 ga.Vec2d, offset, scale, horizon float64, pmap ga.Vec2d) {
...
func (c *Terrain) DrawFrontToBack(m *image.RGBA, p ga.Vec2d, phi, height, distance float64, pmap ga.Vec2d) {
	c.clear(m)
	...
	for z < distance {
		...
		pl := ga.Vec2d{-z, -z}
		pr := ga.Vec2d{z, -z}
		pl = vec2.Rotate(pl, phi)
		pr = vec2.Rotate(pr, phi)

		// now we have the left and right bounds of the horizontal line, draw it
		p1 := ga.Vec2d{p.X + pl.X, p.Y + pl.Y}
		p2 := ga.Vec2d{p.X + pr.X, p.Y + pr.Y}
