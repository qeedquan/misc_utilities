/* tic tac toe using negamax, should play perfectly */
...
play(board_t *b)
{
	...
	for (depth = 9;;) {
		...
		if (b->human_turn == b->player) {
			...
			if (sscanf(buffer, "%u", &choice) == EOF) {
				printf("\n");
				exit(0);
			}

			if (choice < 1 || choice > 9 || ((b->side[!b->player] | b->side[b->player]) & BIT(choice - 1))) {
				printf("\nInvalid Move! Try again!\n\n");
				continue;
			}
			b->side[b->player] |= BIT(choice - 1);
		} else {
			printf("Pondering...\n");
		...
		if (score == oo || score == -oo) {
			...
			if (b->human_turn == b->player)
				...
				return -1;
		} else if (((b->side[b->player] | b->side[!b->player]) & 0x1FF) == 0x1FF) {
			print_board(b);
			return 0;
		}
...
static uint
gen_moves(board_t *b, uint *moves, uint ptm)
{
	uint i, v, lsb;
...
static void
make_move(board_t *b, uint move, uint *undo, uint ptm)
{
	*undo = b->side[ptm];
	b->side[ptm] |= move;
}
...
static void
undo_move(board_t *b, uint move, uint ptm)
{
	b->side[ptm] = move;
}

static int
eval(board_t *b, uint ptm)
{
...
static int
search(board_t *b, int depth, uint ptm)
{
