...
static inline Point calcPoint(Point2f center, double radius, double angle)
{
	return center + Point2f(cos(angle), -sin(angle)) * radius;
}
...
int main(int, char **)
{
	...
	{
		// The model is:
		// phi(t+1) = phi(t) + delta_phi(t)
		// delta_phi(t+1) = delta_phi(t) (constant speed)
		img = Scalar::all(0);
		...
		for (;;)
		{
			...
			float radius = img.cols / 3.f;
			double stateAngle = state.at<float>(0);
			Point statePt = calcPoint(center, radius, stateAngle);

			Mat prediction = KF.predict();
			double predictAngle = prediction.at<float>(0);
			Point predictPt = calcPoint(center, radius, predictAngle);
			...
			Point measPt = calcPoint(center, radius, measAngle);
			...
			Point improvedPt = calcPoint(center, radius, improvedAngle);
			...
			drawMarker(img, calcPoint(center, radius, Mat(KF.transitionMatrix * KF.statePost).at<float>(0)),
