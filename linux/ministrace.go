...
// based on https://blog.nelhage.com/2010/08/write-yourself-an-strace-in-70-lines-of-code/
package main
...
func usage() {
	fmt.Fprintln(os.Stderr, "usage: ministrace program [args] ...")
	flag.PrintDefaults()
	os.Exit(2)
}
...
func xsys(ax uint64) string {
	var tab []string
	switch runtime.GOARCH {
	case "amd64":
		tab = syscalls_amd64
	}
	if ax >= uint64(len(tab)) {
		return fmt.Sprintf("syscall(%d)", ax)
	}
	return fmt.Sprintf("%s(%d)", tab[ax], ax)
}

var syscalls_amd64 = []string{
	0:   "read",
	1:   "write",
	2:   "open",
	3:   "close",
