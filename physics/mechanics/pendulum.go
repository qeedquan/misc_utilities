...
func reset() {
	pendulum = newPendulum(ga.Vec2d{width / 2, 0}, 175)
	fps.SetRate(60)
}
...
func newPendulum(origin ga.Vec2d, radius float64) *Pendulum {
	p := &Pendulum{
		Origin:     origin,
		Radius:     radius,
		Angle:      math.Pi / 4,
...
func (p *Pendulum) Clicked(mx, my float64) {
	m := ga.Vec2d{mx, my}
	d := vec2.Len(vec2.Sub(p.Position, m))
	if d < p.BallRadius {
		p.Dragging = true
	}
}
...
func (p *Pendulum) Drag() {
	if p.Dragging {
		m := ga.Vec2d{mouseX, mouseY}
		d := vec2.Sub(p.Origin, m)
		// angle relative to vertical axis
		p.Angle = math.Atan2(-1*d.Y, d.X) - ga.Deg2Rad(90.0)
	}
}
...
func (p *Pendulum) Display() {
	...
	p.Position = vec2.Add(p.Position, p.Origin)
