...
Outputs a pure tone (one sine wave of fixed frequency)
The volume adjustment can give a distinctly sounding tone to the human ear,
even though it is at the same frequency

To output a pure tone sounding like old computers (such as BASIC BEEP/PLAY/etc), adjust the volume factor to be 3
...
func main() {
	...
	flag.Float64Var(&opt.Volume, "v", opt.Volume, "volume")
	flag.Parse()
	initsdl()
	initsynth()
...
func audiofill(b []byte) {
	...
	for i := 0; i < len(b); {
		s := pure.Sample(timer.Tick) * 0.5 * opt.Volume
		v := int16(ga.LinearRemap(s, -1, 1, math.MinInt16, math.MaxInt16))
