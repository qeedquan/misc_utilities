...
{
	return (float3){
	    .rd = sqrtf(a.x * a.x + a.y * a.y + a.z * a.z),
	    .t = atan2f(sqrtf(a.x * a.x + a.y * a.y), a.z),
	    .p = atan2f(a.y, a.x),
	};
}
...
qslerp(float t, float4 a, float4 b)
{
	...
	c = sub4(b, scale4(a, d));
	c = normalize4(c);
	return add4(scale4(a, cosf(r)), scale4(c, sinf(r)));
}
...
roots3rf(float pa, float pb, float pc, float pd, float z[3])
{
	...
	if (r2 <= (q3 + eps)) {
		...
		q = -2 * sqrtf(q);
		z[0] = q * cosf(t / 3) - a;
		z[1] = q * cosf((t + M_TAU) / 3) - a;
		z[2] = q * cosf((t - M_TAU) / 3) - a;
		return 3;
	}

	float A = -ncbrtf(fabsf(r) + sqrtf(r2 - q3));
	if (r < 0)
		A = -A;
	float B = 0;
	if (A != 0)
