...
func eq(a, b chroma.Float4) bool {
	const eps = 1e-8
...
func average(a chroma.Float4) float64 {
	return (a[0] + a[1] + a[2]) / 3
}

func mul(m *imageutil.Float, s float64) {
	for i := range m.Pix {
		m.Pix[i] = chroma.Float4{
			m.Pix[i][0] * s,
			m.Pix[i][1] * s,
			m.Pix[i][2] * s,
			m.Pix[i][3],
...
func mix(m *imageutil.Float, c chroma.Float4) {
	for i := range m.Pix {
		m.Pix[i][0] *= c[0]
		m.Pix[i][1] *= c[1]
		m.Pix[i][2] *= c[2]
